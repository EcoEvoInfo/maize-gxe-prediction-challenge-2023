...
```{r}
lds <- read.csv('genetics/AgAdapt_LDS_v0.2_Training.csv', header = T)
lds2 <- read.csv('genetics/AgAdapt_LDS_v0.2_Testing.csv', header = T)
lds <- rbind.data.frame(lds, lds2)

lds <- lds %>% unique() %>% na.omit()

gxesnps <- read.csv('genetics/GxE_snps.csv', header = T, check.names = F)
gxesnps <- gxesnps[, !duplicated(colnames(gxesnps))]
colnames(gxesnps)[1] <- "Hybrid"

df_g <- left_join(lds, gxesnps)
df_g <- select(df_g, -Yield_Mg_ha)
```
...
df_yge <- left_join(df_ye, df_g)
```

Add weather data & write to file:
```{r}
weather <- read.table(paste0('weather/reduced_weather_features/loyo',loyo,'_weather_reduced.txt', sep = ""), header = F)
...
write.csv(df,file = paste0('combo_dfs/combo_loyo_',loyo,'.csv'), quote = F, row.names = F)
```
